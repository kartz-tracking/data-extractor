...
/**
 * Whether the person looking at this may write to it.
 *
 * Asking Google who may edit a file means asking Drive, and this add-on deliberately cannot:
 * the only thing it was given is this one spreadsheet, not who it belongs to. So the question
 * is answered the other way round — yes, unless it can be *proved* otherwise — and a write by
 * somebody who may not do it fails with Google's own refusal, which the dialog shows.
 *
 * The alternative, assuming no, told every reader they were a viewer.
 */
function canEdit() {
  try {
    var ss = SpreadsheetApp.getActive();
    var me = Session.getEffectiveUser().getEmail();
    if (!me) return true;                       // no identity to compare: let the write speak
    var owner = ss.getOwner();
    if (owner && owner.getEmail() === me) return true;
    return ss.getEditors().map(function (u) { return u.getEmail(); }).indexOf(me) !== -1;
  } catch (err) {
    return true;
  }
}
