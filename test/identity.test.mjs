...
bad = await threw(async () => call(await token({ aud: 'some-other-app.apps.googleusercontent.com' }), { SCRIPT_AUD: AUD }),
                  'not one of the 1 script');
ok('a valid Google token for another app is refused — this is the whole gate', bad.ok, bad);

const SECOND = '999-second-sheet.apps.googleusercontent.com';
who = await call(await token({ aud: SECOND }), { SCRIPT_AUD: AUD + ', ' + SECOND });
ok('the same add-on in a second spreadsheet is a second id, and both are let in',
   who.email === 'glitter@gmail.com', who);
who = await call(await token(), { SCRIPT_AUD: AUD + '\n' + SECOND });
ok('however the list was pasted in', who.email === 'glitter@gmail.com');
bad = await threw(async () => call(await token({ aud: 'third-app' }), { SCRIPT_AUD: AUD + ',' + SECOND }),
                  'not one of the 2 scripts');
ok('and a third, unlisted one is still refused', bad.ok, bad);
