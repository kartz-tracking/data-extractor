 ...
 *
 *   SCRIPT_AUD       required — the OAuth client id of the add-on. The first call tells you it.
 *                    A comma-separated list, for the same add-on copied into several
 *                    spreadsheets: each copy is its own script project with its own client id.
 *   ALLOWED_EMAILS   optional — a comma-separated list, if the spreadsheets' sharing is not
 *                    tight enough on its own.
 */
import { HttpError, str } from './util.js';

// A setting that may name one thing or several, written however it was pasted in.
const list = v => str(v).split(/[\s,]+/).map(x => x.trim()).filter(Boolean);
...
export async function whoIsCalling(request, env) {
  ...
  // One id, or several: the same add-on copied into another spreadsheet is a new script
  // project with a client id of its own, and all of them are yours.
  const allowed = list(env.SCRIPT_AUD);
  if (!allowed.length) {
  ...
  if (!allowed.includes(claims.aud)) {
    throw new HttpError(403,
      `that identity was minted for ${claims.aud}, which is not one of the ${allowed.length} `
      + `script${allowed.length === 1 ? '' : 's'} this Worker answers to. Adding a copy of the `
      + 'add-on to another spreadsheet? Put its client id on the SCRIPT_AUD list too.');
  }

  const people = list(env.ALLOWED_EMAILS).map(e => e.toLowerCase());
  if (people.length && !people.includes(email))
